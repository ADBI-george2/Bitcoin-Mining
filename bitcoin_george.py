...
def similarity(a, b):
    def mean_std(vec):
        ...
        vec_std = math.sqrt(sum([(vec_z - vec_mean)**2 for vec_z in vec]) / (len(vec)))
        return vec_mean, vec_std

    assert len(a) == len(b), "Vectors are not of same length"
    m = len(a)
    a_mean, a_std = mean_std(a)
    b_mean, b_std = mean_std(b)
    num = sum([(a_z - a_mean)*(b_z - b_mean) for a_z, b_z in zip(a, b)])
    den = m * a_std * b_std
    return num / den
...
def computeDelta(wt, X, Xi):
    ...
    for x_i in Xi.iterrows():
      x_i_vals = x_i[1].tolist()[:-1]
      sim = math.exp(wt * similarity(X_vals, x_i_vals))
      num += x_i[1]["Yi"] * sim
      den += sim
    return num / den
...
# YOUR CODE HERE
model = smf.ols(formula='deltaP ~ deltaP90 + deltaP180 + deltaP360', data=d).fit()
